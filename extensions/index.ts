...
function parseRalphArgs(rawArgs: string): ParsedRalphArgs {
	const args = parseCommandArgs(rawArgs.trim());

	if (args.length === 0) {
		return {
			iterations: DEFAULT_RALPH_ITERATIONS,
			error: "Usage: /ralph [iterations] <prompt-file> | /ralph [iterations] -p <prompt>",
		};
	}
	...
	// /ralph -p "do this"
	// /ralph 10 -p "do this"
	if (mode === "-p" || mode === "--prompt") {
		const promptText = args.slice(index + 1).join(" ").trim();
	...
	// Otherwise it's a prompt file.
	const promptPath = args.slice(index).join(" ").trim();

	if (!promptPath) {
		return {
			iterations,
			error: "Prompt file cannot be empty.",
		};
	}
...
function buildLoopStatus(
	...
	return `Ralph Wiggum loop ${iteration}/${totalIterations}: ${phase} (${promptLabel})`;
}
...
async function writeRalphStateFile(
	...
	const content = buildRalphContent(promptLabel, iteration, totalIterations, iterationSummaries);
	...
	// Per-iteration session history JSONL: one AgentMessage per line.
	// Each iteration gets its own file while sharing the same invocation prefix.
...
export default function ralph(pi: ExtensionAPI) {
	...
	pi.registerCommand("ralph", {
		description:
			"Run a Ralph Wiggum loop with an optional iteration count and prompt file or inline prompt, or stop after the current iteration with /ralph stop",
		...
		handler: async (args, ctx) => {
			...
			if (args.trim() === "stop") {
				...
				ctx.ui.notify(
					"Ralph Wiggum loop will stop after the current iteration completes",
					"info",
				);
			...
			if (parsed.error || (!parsed.promptPath && !parsed.promptText)) {
				ctx.ui.notify(
					parsed.error ??
						"Usage: /ralph [iterations] <prompt-file> | /ralph [iterations] -p <prompt>",
					"warning",
				);
				return;
			}
			...
			// File:
			//   - read literally
			//   - NO prompt template / skill expansion
			//
			// Inline:
			//   - use literal inline prompt
			//   - ENABLE prompt template / skill expansion
			// ---------------------------------------------------------------
			...
			if (parsed.promptText !== undefined) {
				// Inline prompt:
				// /ralph -p "/skill:foo ..."
				trimmedPrompt = parsed.promptText.trim();
				...
				if (!canReadPath(promptPath, ctx.cwd)) {
					ctx.ui.notify(
						`Prompt file must be inside ${ctx.cwd}, ~/.pi, or ~/.local`,
						"error",
					);
					return;
				}

				const resolvedPromptPath = normalizeFsPath(promptPath, ctx.cwd);

				try {
					trimmedPrompt = (await fs.readFile(resolvedPromptPath, "utf8")).trim();
				} catch (error) {
					ctx.ui.notify(
			...
			activeCtx.ui.notify(
				`Starting Ralph Wiggum loop: ${parsed.iterations} iteration${
					parsed.iterations === 1 ? "" : "s"
				}`,
				"info",
			);
			...
			try {
				...
				// Timestamp fixed once per invocation; all iterations of this run
				// share the same per-invocation file path derived from this value.
				const invocationTimestamp = new Date();

				// Capture the original session once so every iteration branches
				// from the same clean parent, regardless of command-context
				// staleness after the first newSession() call.
				...
				for (let i = 1; i <= parsed.iterations; i++) {
					...
					// new session's runner (inside bindExtensions).
					const state = getRalphState();
					...
					const newSessionResult = await activeCtx.newSession({
						...
						withSession: async (replacementCtx) => {
							...
							// IMPORTANT:
							//
							// expandPromptTemplates is true only for inline `-p`
							// prompts. File prompts remain literal.
					...
					if (newSessionResult.cancelled) {
						activeCtx.ui.notify(
							`Ralph Wiggum loop cancelled before iteration ${i}`,
							"warning",
						);
						return;
					}
					...
					activeCtx.ui.notify(
						`Ralph iteration ${i}/${parsed.iterations} complete`,
						"info",
					);
					...
					if (i < parsed.iterations) {
						...
						if (state.stopRequested) {
							...
							activeCtx.ui.notify(
								`Ralph Wiggum loop stopping after ${i}/${parsed.iterations}`,
								"info",
							);
			...
			if (stoppedEarly) {
				activeCtx.ui.notify(
					`Ralph Wiggum loop stopped after ${completedIterations} of ${
						parsed.iterations
					} iteration${parsed.iterations === 1 ? "" : "s"}`,
					"success",
				);
				return;
			}
			...
			activeCtx.ui.notify(
				`Ralph Wiggum loop complete (${parsed.iterations} iteration${
					parsed.iterations === 1 ? "" : "s"
				})`,
				"success",
			);
